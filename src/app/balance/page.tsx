...
export default function Balance() {
  ...
  useEffect(() => {
    if (isUserAuth) {
      // @ts-ignore
      dispatch(getUserWheelData()).then(
        (balanceUserData: TReportCheckup | null) => {
          if (
            balanceUserData &&
            Math.max(...Object.values(balanceUserData)) < 1
          ) {
  ...
  return (
    <ProtectedRoute>
      <Header />
      {userData &&
      USER_RANKS.indexOf(userData.rank_name) <
        USER_RANKS.indexOf('resident') ? (
        <div className="w-full max-w-[1400px] mx-auto h-[calc(100vh-90px)] flex justify-center items-center">
          <BaseSecondaryCard className="max-w-[300px] max-h-[200px] text-white flex justify-center items-center text-center text-text-m-bold">
            Для открытия &quot;Чекапа&quot;, вам необходимо достигнуть ранг
            &quot;Резидент&quot;
          </BaseSecondaryCard>
        </div>
      ) : (
        <>{balanceData ? <BalancePage /> : null}</>
      )}
    </ProtectedRoute>
  );
}
