...
const StartPage = () => {
  ...
  const { userData, isUserPaidSubscription } = useAppSelector(
    (state) => state.user,
  );
  // @ts-ignore
    ...
    <section className="w-full overflow-hidden">
      <div className="min-h-[calc(100vh-90px)] flex flex-col items-center gap-12 justify-center relative w-full bg-bg-gray rounded-9 p-10 max-w-[1400px] mx-auto s_lg:rounded-t-[0px] s_lg:pt-0 s_lg:p-6 pb-3 s_lg:pb-12">
        <div className="flex flex-col items-center gap-6">
          <Link
            href={'/'}
            className="flex items-center gap-2 px-6 ss_lg:hidden"
          >
            <Image
        ...
        <div className="w-full flex flex-col justify-between items-center relative max-w-[560px] min-h-[240px] gap-6 pt-12 pb-10 px-12 bg-white rounded-4">
          {step === 0 && isAuthCheck ? (
            <>
              <p className="text-text-m-bold text-center">
                Вам необходимо авторизоваться, чтобы создать профиль в системе
                Rec’s community
              </p>
          ...
          {step === 1 && isAuthCheck ? (
            <>
              ...
              <p className="text-text-m-bold text-center">
                Для этого зарегистрируйся в боте и напиши
                &quot;Авторизация&quot;, после чего напиши свой логин и пароль
                от Rec’s community
              </p>
          ...
          {step === 2 && isAuthCheck ? (
            <>
              <p className="text-text-m-bold text-center">
                Последний шаг - оплатить подписку <br />И дождаться пока ее
                примет Recs
              </p>
