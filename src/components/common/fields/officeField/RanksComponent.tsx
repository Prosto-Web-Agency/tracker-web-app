...
export default function RanksComponent({ userRank }: TRanksComponent) {
  ...
  return (
    ...
    >
      ...
      >
        <RankComponent
          active={USER_RANKS.indexOf(userRank) >= USER_RANKS.indexOf('empty')}
          rank="empty"
          onClick={() => {}}
        />
        <RankComponent
          active={USER_RANKS.indexOf(userRank) >= USER_RANKS.indexOf('new')}
          rank="new"
          onClick={() => {}}
        />
        <RankComponent
          active={
            USER_RANKS.indexOf(userRank) >= USER_RANKS.indexOf('resident')
          }
          rank="resident"
          onClick={() => setResidentModalOpen(true)}
        />
        <RankComponent
          active={USER_RANKS.indexOf(userRank) >= USER_RANKS.indexOf('expert')}
          rank="expert"
          onClick={() => setExpertModalOpen(true)}
        />
        <RankComponent
          active={
            USER_RANKS.indexOf(userRank) >= USER_RANKS.indexOf('ambassador')
          }
          rank="ambassador"
          onClick={() => setAmbassadorModalOpen(true)}
        />
        <RankComponent
          active={USER_RANKS.indexOf(userRank) >= USER_RANKS.indexOf('adviser')}
          rank="adviser"
          onClick={() => setAdvizerModalOpen(true)}
        />
        <RankComponent
          active={
            USER_RANKS.indexOf(userRank) >= USER_RANKS.indexOf('headliner')
          }
          rank="headliner"
          onClick={() => setHeadlinerModalOpen(true)}
        />
      </div>

      <ModalExpert open={isExpertModalOpen} onClose={handleCloseExpertModal} />
      <ModalAdvizer
        open={isAdvizerModalOpen}
        onClose={handleCloseAdvizerModal}
      />
      <ModalAmbassador
        open={isAmbassadorModalOpen}
        onClose={handleCloseAmbassadorModal}
      />
      <ModalHeadliner
        open={isHeadlinerModalOpen}
        onClose={handleCloseHeadlinerModal}
      />
      <ModalResident
