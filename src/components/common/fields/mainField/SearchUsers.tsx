...
export default function SearchUsers() {
  ...
  const { searchUsers }: { searchUsers: TUserSearchDataArray } = useSelector(
    //@ts-ignore
  ...
  function handleSubscribeOnUser(login: string, is_subscribe: boolean) {
    if (is_subscribe) {
      ...
      //@ts-ignore
      dispatch(subscribeOnUserByLogin(login)).then(() => {
        //@ts-ignore
        dispatch(getSearchUsersThunk(text));
      });
    }
  ...
  function handleUnsubscribeOnUser(login: string) {
    //@ts-ignore
    dispatch(unsubscribeOnUserByLogin(login)).then(() => {
      //@ts-ignore
      dispatch(getSearchUsersThunk(text));
    });
  }
  ...
  return (
    <div className="relative">
      ...
      {text === '' ? null : (
        <div className="flex flex-col gap-2 bg-white rounded-4 z-[100] shadow-primary mt-4 p-6 min-h-[100px] max-h-[240px] w-full absolute">
          {searchUsers.map(
            (
              {
                first_name,
              ...
              <div
                className={
                  'flex justify-between items-center gap-2 w-full cursor-pointer hover:bg-bg-gray transition rounded-4'
                }
