...
function ChatSmallComponent() {
  ...
  useEffect(() => {
    setWebsocket(
      new WebSocket(String(process.env.WEBSOCKET) + storage.get(TOKEN) ?? ''),
    );
    ...
    <>
      {fullScreen ? (
        ...
        >
          ...
          >
            <CardHeader title={'Чат'} size={'small'} />
          </div>
          ...
          <div
            ...
            onMouseLeave={() => setHover(false)}
            className={classNames(
              'absolute flex justify-center items-center top-0 left-0 transition bg-black h-full w-full',
              {
          ...
          >
            <p className="text-white text-heading-ss">
